...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        # 关联强度
        sa.Column(
            "relevance_score", sa.Numeric(3, 2), nullable=False, server_default="0.5"
        ),
        ...
        # 唯一约束
        sa.UniqueConstraint(
            "mistake_id", "knowledge_point_id", name="uq_mistake_knowledge"
        ),
    )
    ...
    op.create_index(
        "idx_mkp_knowledge_point", "mistake_knowledge_points", ["knowledge_point_id"]
    )
    op.create_index(
        "idx_mkp_primary",
        "mistake_knowledge_points",
    ...
    op.create_table(
        ...
        # 图谱数据 (JSON)
        sa.Column(
            "knowledge_points", postgresql.JSON(astext_type=sa.Text()), nullable=False
        ),
        sa.Column("weak_chains", postgresql.JSON(astext_type=sa.Text()), nullable=True),
        sa.Column(
            "strong_areas", postgresql.JSON(astext_type=sa.Text()), nullable=True
        ),
        # 学情摘要
        ...
        sa.UniqueConstraint(
            "user_id", "subject", "snapshot_date", name="uq_user_subject_date"
        ),
    )

    # 创建索引
    op.create_index(
        "idx_ukgs_user_subject",
        "user_knowledge_graph_snapshots",
        ["user_id", "subject"],
    )
    op.create_index(
    ...
    op.create_table(
        ...
        # 唯一约束
        sa.UniqueConstraint(
            "user_id", "knowledge_point_id", name="uq_user_knowledge_point"
        ),
    )
    ...
    op.create_index(
        "idx_kplt_kp", "knowledge_point_learning_tracks", ["knowledge_point_id"]
    )
