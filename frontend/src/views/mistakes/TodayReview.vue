...
import { ElMessage } from 'element-plus'
import { getTodayReviewTasks, completeReview } from '@/api/mistakes'
import type { TodayReviewTask, ReviewCompleteRequest } from '@/types/mistake'
...
const loadTodayTasks = async () => {
  loading.value = true
  try {
    todayTasks.value = await getTodayReviewTasks()
  } catch (error) {
    ElMessage.error('加载复习任务失败')
    console.error(error)
  } finally {
...
const submitReview = async () => {
  ...
  submitting.value = true
  try {
    const response = await completeReview(currentTask.value.mistake_id, reviewForm.value)

    ElMessage.success(reviewForm.value.is_correct ? '复习完成！继续加油！' : '没关系，再接再厉！')

    // 更新任务状态
    const taskIndex = todayTasks.value.findIndex(
      (t) => t.mistake_id === currentTask.value!.mistake_id
    )
    if (taskIndex !== -1) {
      todayTasks.value[taskIndex].is_completed = true
      todayTasks.value[taskIndex].mastery_status = response.mastery_status
      todayTasks.value[taskIndex].review_count += 1
      if (reviewForm.value.is_correct) {
        todayTasks.value[taskIndex].correct_count += 1
      }
    }
