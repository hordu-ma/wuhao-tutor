  ...
  <div class="knowledge-radar-chart">
    <div class="bg-white rounded-lg p-6 shadow-sm border border-gray-200">
      ...
      <div v-if="showDetail" class="mt-6">
        ...
        <el-table :data="tableData" size="small" :max-height="300" style="width: 100%">
          ...
          <el-table-column prop="correctRate" label="正确率" width="100">
            <template #default="{ row }">
              <span :class="getCorrectRateClass(row.correctRate)"> {{ row.correctRate }}% </span>
            </template>
          </el-table-column>
...
const filteredKnowledgePoints = computed(() => {
  ...
  // 按掌握度排序，优先显示薄弱的知识点
  return points.sort((a, b) => a.masteryLevel - b.masteryLevel).slice(0, props.maxPoints)
})
...
const radarData = computed(() => {
  ...
  const indicator = points.map((point) => ({
    name:
      point.name && point.name.length > 6
  ...
  const seriesData = [
    {
      name: '掌握度',
      value: points.map((point) => point.masteryLevel || 0),
      areaStyle: {
        color: {
          type: 'radial',
          x: 0.5,
  ...
  // 如果显示对比，添加正确率数据
  if (props.showComparison && points.length > 0) {
    seriesData.push({
      name: '正确率',
      value: points.map((point) => point.correctRate || 0),
      areaStyle: {
        color: {
          type: 'radial',
          x: 0.5,
...
const initRadarChart = () => {
  ...
  if (!indicator || indicator.length === 0) {
    ...
    radarChartInstance.setOption(emptyOption as any)
    return
  }
        ...
        try {
          ...
          return `
            <div style="padding: 12px; max-width: 280px;">
              <div style="font-weight: 600; margin-bottom: 8px; color: #111827;">${point.name || '知识点'}</div>
              <div style="margin-bottom: 4px; color: #6b7280;">学科: <span style="color: #3b82f6;">${point.subject || '未知'}</span></div>
              <div style="margin-bottom: 4px; color: #6b7280;">掌握度: <span style="color: ${(point.masteryLevel || 0) >= 80 ? '#10b981' : (point.masteryLevel || 0) >= 60 ? '#f59e0b' : '#ef4444'}; font-weight: 500;">${point.masteryLevel || 0}%</span></div>
              <div style="margin-bottom: 4px; color: #6b7280;">正确率: <span style="color: ${(point.correctRate || 0) >= 80 ? '#10b981' : (point.correctRate || 0) >= 60 ? '#f59e0b' : '#ef4444'}; font-weight: 500;">${point.correctRate || 0}%</span></div>
              <div style="margin-bottom: 4px; color: #6b7280;">练习次数: <span style="color: #374151; font-weight: 500;">${point.practiceCount || 0}次</span></div>
              <div style="color: #6b7280;">难度: <span style="color: ${point.difficulty === 'easy' ? '#10b981' : point.difficulty === 'medium' ? '#f59e0b' : '#ef4444'}; font-weight: 500;">${getDifficultyText(point.difficulty || 'medium')}</span></div>
            </div>
          `
        } catch (error) {
