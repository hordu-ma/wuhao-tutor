...
export const useHomeworkStore = defineStore('homework', () => {
   ...
   * 获取作业列表
   */
  const getHomeworkList = async (params?: HomeworkQueryParams, append = false) => {
    try {
   ...
   * 提交作业
   */
  const submitHomework = async (data: HomeworkSubmitRequest): Promise<HomeworkRecord | null> => {
    try {
   ...
   * 重命名作业
   */
  const renameHomework = async (homeworkId: string, newTitle: string) => {
    try {
      await homeworkAPI.renameHomework(homeworkId, newTitle)
      ...
      // 更新当前作业详情
      if (currentHomework.value?.id === homeworkId) {
        currentHomework.value.title = newTitle
        currentHomework.value.updated_at = new Date().toISOString()
      }

      ElMessage.success('重命名成功')
    } catch (error) {
      console.error('重命名作业失败:', error)
      ElMessage.error('重命名作业失败')
      throw error
    }
  }
  ...
  const batchDeleteHomework = async (homeworkIds: string[]) => {
    try {
      ...
      // 从列表中移除
      homeworkList.value = homeworkList.value.filter((h) => !homeworkIds.includes(h.id))
  ...
  const updateHomework = async (
    ...
    try {
      const updatedHomework = await homeworkAPI.updateHomework(homeworkId, data)
