...
export class HomeworkAPI {
  // API基础路径
  private readonly baseURL = "/api/v1/homework";
  ...
  async submitHomework(data: HomeworkSubmitRequest): Promise<HomeworkRecord> {
    ...
    const response = await http.post<HomeworkRecord>(
      `${this.baseURL}/submit`,
      formData,
      {
        headers: {
  ...
  async getHomework(homeworkId: string): Promise<HomeworkRecord> {
    const response = await http.get<HomeworkRecord>(
      `${this.baseURL}/${homeworkId}`
    );
    return response;
  }
  ...
  async getHomeworkList(
    ...
    const response = await http.get<HomeworkListResponse>(
      `${this.baseURL}/list`,
      {
        params,
      }
  ...
  async updateHomework(
    ...
    const response = await http.put<HomeworkRecord>(
      `${this.baseURL}/${homeworkId}`,
      data
    );
    return response;
