...
// 创建学习目标
export const createLearningGoal = (goal: Omit<LearningGoal, 'id' | 'createdAt' | 'currentValue'>) => {
...
// 生成学习报告
export const generateLearningReport = (type: 'daily' | 'weekly' | 'monthly', startDate: string, endDate: string) => {
...
// 获取学习排行榜
export const getLeaderboard = (type: 'study_time' | 'homework_count' | 'avg_score' | 'streak', period: string = 'week') => {
...
// 更新学习提醒
export const updateStudyReminder = (id: string, updates: {
  time?: string
  message?: string
  enabled?: boolean
}) => {
