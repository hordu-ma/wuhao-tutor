# 五好伴学 (Wuhao Tutor)

> 基于阿里云百炼 AI 的 K12 智能学习平台

一个现代化教育科技平台，为 K12 学生打造，提供 AI 智能问答、错题管理、知识图谱和学情分析服务。

![Python](https://img.shields.io/badge/python-3.12+-blue.svg)
![FastAPI](https://img.shields.io/badge/FastAPI-0.104+-green.svg)
![Vue](https://img.shields.io/badge/Vue-3.4+-4FC08D.svg)
![Status](https://img.shields.io/badge/status-Production-green.svg)

---

## 📊 项目现状

**生产环境**: https://www.horsduroot.com  
**服务器**: 121.199.173.244 (Nginx + systemd + HTTPS)  
**技术架构**: FastAPI + Vue3 + 微信小程序 + PostgreSQL + Redis + 阿里云百炼  
**当前版本**: v0.1.0

### 核心功能

| 模块           | 状态        | 说明                                     |
| -------------- | ----------- | ---------------------------------------- |
| **作业问答**   | ✅ 生产可用 | AI 对话式问答、数学公式渲染、流式响应    |
| **错题手册**   | ✅ 生产可用 | 错题记录、智能复习提醒、艾宾浩斯算法     |
| **知识图谱**   | ✅ 生产可用 | 知识点关联分析、薄弱点识别、学习路径推荐 |
| **学习分析**   | ✅ 生产可用 | 学习时长统计、知识点掌握度、趋势分析     |
| **微信小程序** | ✅ 已上线   | 完整功能覆盖，连接生产环境               |

### 📚 关键文档

- **[DEVELOPMENT_CONTEXT.md](DEVELOPMENT_CONTEXT.md)** - 项目开发上下文（Phase 1-7 已完成）
- **[MISTAKE_EXTRACTION_OPTIMIZATION.md](docs/MISTAKE_EXTRACTION_OPTIMIZATION.md)** - 错题本逼题提取方案
- **[部署文档](docs/deployment/)** - 生产部署指南

---

## ✨ 核心特性

### 💬 AI 智能问答

- **多模态输入**: 文字、图片 OCR、语音识别
- **流式响应**: Server-Sent Events (SSE) 实时输出
- **公式渲染**: KaTeX 支持数学公式显示
- **上下文感知**: 结合学情提供个性化答案

**API 端点**: 25+ 个

### 📚 智能错题管理

- **复习算法**: 艾宾浩斯遗忘曲线算法
- **知识关联**: 自动提取知识点标签
- **掌握度跟踪**: 0-100 分段评分
- **今日复习**: 每日自动生成待复习清单

**API 端点**: 10+ 个

### 🕸️ 知识图谱

- **知识点提取**: AI 自动识别知识点
- **关系建模**: 前置关系、并列关系、应用关系
- **薄弱点分析**: 基于掌握度和错误率
- **可视化展示**: ECharts 图谱交互

**API 端点**: 3+ 个

### 📊 学习分析

- **多维度统计**: 学习时长、提问次数、活跃天数
- **知识掌握**: 基于答题记录的掌握度分析
- **学科分布**: 各学科学习占比和趋势

**API 端点**: 8+ 个

### 🔒 安全特性

**限流保护** (三层):

```
IP 级别:     100 请求/分钟 (Token Bucket)
用户级别:   50 请求/分钟 (Token Bucket)
AI 服务:    20 请求/分钟 (Sliding Window)
登录端点:   10 请求/分钟 (Token Bucket)
```

**认证机制**: JWT 双 Token (Access 8 天 + Refresh 30 天)

**性能监控**: 慢查询监听 (>1.0s)、N+1 检测、系统资源追踪

---

## 🏭 技术架构

### 多端架构

```
┌─────────────────────────────────┐
│     客户端层 (Client Tier)      │
├─────────────────────────────────┤
│  Web 前端  │  微信小程序          │
│  Vue3 + TS  │  原生 JS + TS       │
└─────────────────────────────────┘
            ↓  HTTPS / API
┌─────────────────────────────────┐
│    API 网关层 (Nginx)         │
│  SSL/TLS + 静态托管 + 路由   │
└─────────────────────────────────┘
            ↓
┌─────────────────────────────────┐
│   FastAPI 后端 (Uvicorn)     │
│  四层架构 + 异步 + 中间件   │
└─────────────────────────────────┘
     ↓           ↓          ↓
┌───────────┐ ┌─────────┐ ┌─────────┐
│PostgreSQL│ │ Redis  │ │阿里云AI│
│  数据库  │ │  缓存  │ │百炼+OSS│
└───────────┘ └─────────┘ └─────────┘
```

### 技术栈

**后端**:

- Python 3.12+ | FastAPI 0.104+ | SQLAlchemy 2.x (Async)
- Alembic 迁移 | Pydantic v2 | JWT 认证

**数据库**:

- 生产: PostgreSQL 14+ | 开发: SQLite
- 缓存: Redis 6+ (限流 + Token 存储)

**AI 服务**:

- 阿里云百炼 (通义千问) | 语音识别 ASR | OSS 存储

**前端**:

- Vue 3.4+ (Composition API) | TypeScript 5.6+
- Element Plus 2.5+ | Vite 5+ | Pinia + Vue Router
- KaTeX (公式) | ECharts (图表)

**小程序**:

- 微信原生 API | TypeScript 声明
- 自定义网络封装 | 本地缓存管理

**部署**:

- 阿里云 ECS | Nginx | systemd | Let's Encrypt

### 四层架构

```
API Layer      →  HTTP 请求处理、参数验证
Service Layer  →  业务逻辑、事务管理
Repository     →  数据访问、复杂查询
Model Layer    →  ORM 映射、数据模型
```

**架构原则**:

- ✅ 严格分层：API → Service → Repository → Model
- ✅ 全异步：所有 I/O 操作 `async/await`
- ✅ 类型安全：mypy strict 模式
- ❗ 禁止跨层：禁止 API 直接调用 Repository

---

## 🚀 快速开始

### 环境要求

```bash
Python 3.12+    # 后端运行环境
Node.js 18+     # 前端构建环境
uv              # Python 包管理器
```

### 安装步骤

#### 1. 克隆项目

```bash
git clone https://github.com/hordu-ma/wuhao-tutor.git
cd wuhao-tutor
```

#### 2. 后端配置

```bash
# 安装 uv
curl -LsSf https://astral.sh/uv/install.sh | sh

# 同步依赖
uv sync

# 复制配置模板
cp config/templates/.env.development .env

# 编辑 .env 配置：
# - BAILIAN_API_KEY=sk-xxx  # 阿里云百炼 API Key
# - SECRET_KEY=xxx          # JWT 密钥
```

#### 3. 初始化数据库

```bash
# 运行迁移
uv run alembic upgrade head
```

#### 4. 启动服务

```bash
# 一键启动前后端
./scripts/start-dev.sh

# 或分别启动
# 后端
uv run python src/main.py

# 前端
cd frontend && npm install && npm run dev
```

#### 5. 访问应用

| 入口         | 地址                         | 说明       |
| ------------ | ---------------------------- | ---------- |
| **Web 前端** | http://localhost:5173        | Vue3 应用  |
| **API 文档** | http://localhost:8000/docs   | Swagger UI |
| **健康检查** | http://localhost:8000/health | 服务状态   |

### 开发工具

```bash
# 后端
make dev              # 启动开发服务器
make test             # 运行测试 + 覆盖率
make lint             # 代码格式化
make db-init          # 数据库迁移

# 前端
cd frontend
npm run dev           # 开发服务器
npm run build         # 生产构建
npm run type-check    # 类型检查
```

---

## 🛠️ 项目结构

```
wuhao-tutor/
├── src/                    # 后端源码
│   ├── api/v1/             # API 路由层 (50+ 端点)
│   ├── services/           # 业务逻辑层
│   ├── repositories/       # 数据访问层
│   ├── models/             # ORM 模型 (15+ 表)
│   ├── schemas/            # Pydantic 模型
│   ├── core/               # 核心基础设施
│   └── main.py             # 应用入口
├── frontend/               # Vue3 前端
├── miniprogram/            # 微信小程序 (15+ 页面)
├── alembic/                # 数据库迁移 (15+ 版本)
├── scripts/                # 开发脚本
│   ├── deploy.sh           # 一键部署
│   ├── start-dev.sh        # 启动开发环境
│   └── rollback.sh         # 紧急回滚
├── tests/                  # 测试套件 (80%+ 覆盖率)
├── docs/                   # 项目文档
└── config/                 # 配置模板
```

---

## 📊 项目统计

```
代码规模: ~44,000 行
- Python 后端: ~20,000 行
- Vue 前端: ~12,000 行
- 微信小程序: ~8,000 行
- 测试代码: ~4,000 行

API 端点: 50+ 个
- 学习问答: 25+ | 错题管理: 10+
- 学习分析: 8+ | 知识图谱: 3+

数据库表: 15+ 个
测试覆盖率: ~82%

性能指标 (生产):
- API 响应: P50 < 100ms, P95 < 200ms
- AI 问答: P50 < 2s, P95 < 3s
```

---

## 🔮 未来规划

### 短期 (1-2 个月)

- [ ] 向量数据库集成 (PGVector)
- [ ] 推荐系统基础版
- [ ] 知识图谱增强
- [ ] 小程序功能增强
- [ ] 性能优化

### 中期 (3-6 个月)

- [ ] RAG 语义增强
- [ ] 协作学习功能
- [ ] 多模态输入增强
- [ ] 学习报告生成

### 长期 (6-12 个月)

- [ ] 多学科扩展
- [ ] 自适应学习
- [ ] 教师管理端
- [ ] 移动端原生应用

---

## 👥 维护者

- **hordu-ma** - [GitHub](https://github.com/hordu-ma)

---

## 📡 联系方式

- 项目地址: https://github.com/hordu-ma/wuhao-tutor
- 问题反馈: https://github.com/hordu-ma/wuhao-tutor/issues
- 生产环境: https://www.horsduroot.com
- 健康检查: https://horsduroot.com/health

---

<div align="center">

**⭐ 如果这个项目对你有帮助,请给它一个 Star! ⭐**

Made with ❤️ by hordu-ma

</div>程序 | 用户信息管理、头像上传、学习统计展示                      |
| **微信小程序**   | ✅ 已上线   | 小程序原生   | 完整功能覆盖，连接生产环境 API (horsduroot.com)           |
| **答题质量评估** | ✅ 生产可用 | Web + 小程序 | AI 驱动的答案质量评分、改进建议                           |
| **语音识别**     | ✅ 生产可用 | 小程序       | 阿里云语音识别，支持语音提问                              |
| **图片识别**     | ✅ 生产可用 | Web + 小程序 | 作业拍照、题目识别、OCR 文字提取                          |

### 📚 关键文档

| 文档                                                        | 说明                                 |
| ----------------------------------------------------------- | ------------------------------------ |
| **[开发进度](DEVELOPMENT_STATUS.md)** ⭐                    | 当前开发状态、已完成功能、下阶段计划 |
| **[错题本优化方案](MISTAKE_EXTRACTION_OPTIMIZATION.md)** 🔥 | 错题本逐题提取开发指导文档（方案 A） |
| **[更新日志](CHANGELOG.md)**                                | 版本更新和功能变更记录               |
| **[开发路线图](DEVELOPMENT_ROADMAP.md)**                    | 长期开发规划（12 个月路线图）        |
| **[文档中心](docs/DOCS-README.md)**                         | 完整的项目文档导航                   |
| **[Copilot 指令](.github/copilot-instructions.md)**         | AI 辅助开发规范                      |

**按类别浏览**:

- 📐 [架构设计](docs/architecture/) - 系统架构、数据模型、API 设计
- 🔌 [API 文档](docs/api/) - 接口文档、模型定义
- 💾 [数据库](docs/database/) - 表结构、迁移脚本
- 📱 [小程序](docs/miniprogram/) - 微信小程序开发文档
- 🌐 [前端](docs/frontend/) - Web 前端开发指南
- 🔧 [解决方案](docs/solutions/) - 常见问题修复方案
- 📖 [使用指南](docs/guide/) - 功能使用说明

### 🎯 近期更新 (2025-11-05)

**重要说明**: 当前微信小程序端直接连接阿里云生产环境 (`https://www.horsduroot.com`)

- ✅ **微信小程序全面上线** - 完整功能覆盖，已连接生产环境，支持错题本、作业问答、学习报告等
- ✅ **知识图谱可视化** - 前后端完整实现，支持知识点关系展示和薄弱点分析
- ✅ **答题质量评估系统** - AI 驱动的答案质量评分和改进建议
- ✅ **语音识别集成** - 阿里云语音识别服务，支持小程序语音提问
- ✅ **图片 OCR 处理** - 作业拍照识别、数学公式提取
- ✅ **公式渲染优化** - KaTeX 集成，支持复杂数学公式展示
- ✅ **性能监控系统** - 完整的性能指标收集、慢查询监控、系统资源追踪
- ✅ **多层限流保护** - IP/用户/AI 服务三层限流，Token Bucket + Sliding Window 双算法

---

## ✨ 核心特性

### 💬 作业问答 (AI 智能学习助手)

**基于阿里云百炼通义千问的智能问答系统，提供上下文感知的学习辅导**

**核心能力**:

- **AI 对话引擎**: 基于阿里云百炼智能体，使用通义千问大模型
- **上下文感知**: 自动维持会话上下文，理解连续提问意图
- **多模态输入**: 支持文字、图片（OCR）、语音（ASR）三种输入方式
- **流式响应**: Server-Sent Events (SSE) 实时流式输出，优化用户体验
- **公式渲染**: KaTeX 完整支持，完美显示数学公式、符号和方程式
- **学情关联**: MCP 上下文服务集成，结合薄弱知识点提供个性化答案
- **会话管理**: 支持多会话并行、会话归档、历史记录搜索和标签分类

**技术实现**:

- 通义千问风格极简 UI (三栏可折叠布局)
- `BailianService` 统一 AI 调用封装，支持重试和超时控制
- `LearningService` 完整业务逻辑层 (~2,400 行)
- Markdown 渲染 + 代码高亮 (highlight.js)
- WebSocket 支持实时双向通信

**API 端点** (25+ 个):

```
POST   /api/v1/learning/ask              # 标准问答
POST   /api/v1/learning/ask-stream       # 流式问答
POST   /api/v1/learning/voice-to-text    # 语音识别
POST   /api/v1/learning/image-ocr        # 图片识别
GET    /api/v1/learning/sessions         # 会话列表
POST   /api/v1/learning/sessions         # 创建会话
GET    /api/v1/learning/sessions/{id}/history  # 会话历史
WebSocket /api/v1/learning/ws/ask        # WebSocket 实时问答
```

### 📚 错题手册 (智能复习系统)

**基于艾宾浩斯遗忘曲线的智能错题管理和复习系统**

**核心功能**:

- **智能记录**: 手动添加或从学习问答中一键记录错题
- **复习算法**: SpacedRepetitionAlgorithm 实现艾宾浩斯遗忘曲线
- **知识关联**: 自动提取知识点标签，构建知识图谱关联
- **掌握度跟踪**: 基于复习次数和正确率的掌握度评分 (0-100)
- **今日复习**: 每日自动生成待复习错题清单
- **统计分析**: 错题分布、掌握度趋势、复习效果追踪、学科分析

**技术架构**:

- `MistakeService` 完整业务逻辑 (~1,260 行)
- `MistakeRepository` + `MistakeReviewRepository` 双仓储模式
- 数据表: `mistake_records` + `mistake_reviews`
- 支持 SQLite (开发) 和 PostgreSQL (生产)

**复习间隔算法**:

```python
复习次数  间隔天数  掌握度阈值
0        1天      60%
1        2天      70%
2        4天      80%
3        7天      85%
4        15天     90%
5+       30天     95%
```

**API 端点** (10+ 个):

```
GET    /api/v1/mistakes              # 错题列表
POST   /api/v1/mistakes              # 创建错题
GET    /api/v1/mistakes/{id}         # 错题详情
PUT    /api/v1/mistakes/{id}         # 更新错题
DELETE /api/v1/mistakes/{id}         # 删除错题
POST   /api/v1/mistakes/{id}/review  # 提交复习
GET    /api/v1/mistakes/today-review # 今日待复习
GET    /api/v1/mistakes/statistics   # 统计分析
```

### 🕸️ 知识图谱 (知识点关联系统)

**智能知识点识别、关联分析和学习路径推荐**

**核心功能**:

- **知识点提取**: AI 自动从错题和问答中提取知识点
- **关系建模**: 前置关系、并列关系、应用关系三种关联类型
- **薄弱点识别**: 基于掌握度和错误率的薄弱知识点分析
- **学习路径**: 根据前置关系生成个性化学习路径
- **可视化展示**: ECharts 图谱可视化，支持节点交互

**数据模型**:

- `KnowledgeNode`: 知识点节点 (学科、难度、掌握度)
- `KnowledgeRelation`: 知识点关系 (关系类型、权重)
- `MistakeKnowledgePoint`: 错题-知识点关联
- `UserKnowledgeGraphSnapshot`: 用户知识图谱快照

**API 端点**:

```
GET    /api/v1/knowledge-graph/user/{user_id}  # 用户知识图谱
GET    /api/v1/knowledge-graph/weak-points     # 薄弱知识点
GET    /api/v1/knowledge-graph/learning-path   # 学习路径推荐
```

### 📊 学习分析 (全维度学情统计)

**多维度学习数据统计、可视化和智能建议**

**分析维度**:

- **学习进度**: 学习时长、提问次数、活跃天数、连续学习天数
- **知识掌握**: 基于答题记录的知识点掌握度分析 (0-100 分)
- **学科分布**: 各学科学习时长占比、平均分、提升趋势
- **时间序列**: 支持日/周/月多粒度数据聚合
- **对比分析**: 同学科历史对比、同年级平均对比

**核心指标**:

```python
学习时长    # 问答总时长 + 复习总时长
提问次数    # 有效问答次数
活跃天数    # 有学习记录的天数
知识点数    # 学习过的知识点总数
掌握度      # 加权平均掌握度 (0-100)
正确率      # 答题正确率百分比
```

**API 端点** (8+ 个):

```
GET    /api/v1/analytics/learning-progress    # 学习进度趋势
GET    /api/v1/analytics/knowledge-mastery    # 知识点掌握
GET    /api/v1/analytics/subject-stats        # 学科统计
GET    /api/v1/analytics/study-time           # 学习时长分析
GET    /api/v1/analytics/weak-subjects        # 薄弱学科
```

### 🎯 答题质量评估

**AI 驱动的答案质量评分和改进建议系统**

**评估维度**:

- 准确性评分 (0-100)
- 完整性评分 (0-100)
- 清晰度评分 (0-100)
- 总体质量分数
- 具体改进建议

**数据模型**: `AnswerQualityScore` 表存储所有评估记录

### 🔒 企业级安全特性

**多层防护体系，保障系统稳定运行**

**限流保护** (三层限流):

```python
# IP 层限流 (防 DDoS)
IP 级别:     100 请求/分钟 (Token Bucket)
# 用户层限流 (防滥用)
用户级别:     50 请求/分钟 (Token Bucket)
# AI 服务限流 (控制成本)
AI 服务:      20 请求/分钟 (Sliding Window)
# 登录端点限流 (防暴力破解)
登录端点:     10 请求/分钟 (Token Bucket)
```

**安全头配置**:

- Content-Security-Policy (CSP)
- X-Frame-Options: DENY
- X-Content-Type-Options: nosniff
- Strict-Transport-Security (HSTS)
- X-XSS-Protection

**认证机制**:

- JWT 双 Token (Access + Refresh)
- Access Token: 8 天有效期
- Refresh Token: 30 天有效期
- 自动续期机制

**性能监控**:

- 实时性能指标收集 (`PerformanceMonitoringMiddleware`)
- 慢查询自动监控 (>1.0 秒自动记录)
- N+1 查询检测和警告
- 系统资源监控 (CPU、内存、磁盘)

**中间件堆栈** (执行顺序):

```
1. PerformanceMonitoringMiddleware  # 性能监控
2. SecurityHeadersMiddleware        # 安全头
3. RateLimitMiddleware              # 限流保护
4. CORSMiddleware                   # 跨域处理
5. TrustedHostMiddleware            # 主机验证
6. LoggingMiddleware                # 日志记录
```

---

## 🏗️ 技术架构

### 多端架构总览

```
┌─────────────────────────────────────────────────────┐
│              客户端层 (Client Tier)                  │
├─────────────────────────────────────────────────────┤
│  Web 前端          │  微信小程序                      │
│  Vue3 + TS        │  原生 JS + TS 声明                │
│  Element Plus     │  微信原生组件                     │
│  Vite 5.x         │  微信开发者工具                   │
│  Port: 5173       │  连接生产环境 API                 │
└─────────────────────────────────────────────────────┘
                        ↓  HTTPS / API Calls
┌─────────────────────────────────────────────────────┐
│            API 网关层 (API Gateway)                  │
├─────────────────────────────────────────────────────┤
│  Nginx 反向代理 (443 → 8000)                         │
│  - SSL/TLS 终止 (Let's Encrypt)                      │
│  - 静态文件托管 (/var/www/html)                      │
│  - 请求路由 (/api → Backend)                         │
│  - Gzip 压缩                                          │
└─────────────────────────────────────────────────────┘
                        ↓
┌─────────────────────────────────────────────────────┐
│           后端服务层 (Backend Services)               │
├─────────────────────────────────────────────────────┤
│  FastAPI 应用 (异步 ASGI)                             │
│  - Uvicorn ASGI 服务器                                │
│  - 中间件堆栈 (监控/安全/限流/日志)                   │
│  - 四层架构 (API/Service/Repository/Model)           │
│  - systemd 服务管理                                   │
│  Port: 8000 (内部)                                    │
└─────────────────────────────────────────────────────┘
         ↓                      ↓                  ↓
┌──────────────┐    ┌──────────────┐    ┌──────────────┐
│ PostgreSQL   │    │   Redis      │    │ 阿里云服务    │
│ 主数据库     │    │   缓存       │    │ - 百炼 AI     │
│ Port: 5432   │    │ Port: 6379   │    │ - 语音识别    │
│              │    │              │    │ - OSS 存储    │
└──────────────┘    └──────────────┘    └──────────────┘
```

### 技术栈详解

```yaml
# ===== 后端技术栈 =====
后端框架:
  语言: Python 3.11+
  框架: FastAPI 0.104+ (异步高性能 Web 框架)
  ASGI 服务器: Uvicorn (标准 ASGI 实现)
  包管理器: uv (快速 Python 包管理)

数据访问:
  ORM: SQLAlchemy 2.x (Async Engine)
  迁移: Alembic 1.12+
  验证: Pydantic v2 (Settings + 数据模型)
  连接池: asyncpg (PostgreSQL) / aiosqlite (SQLite)

认证与安全:
  JWT: python-jose + cryptography
  密码哈希: bcrypt (12 轮加盐)
  Token 类型: Access (8天) + Refresh (30天)
  限流算法: Token Bucket + Sliding Window

# ===== 数据库与缓存 =====
主数据库:
  生产: PostgreSQL 14+ (阿里云 RDS)
  开发: SQLite (本地文件)
  特性: 异步查询、连接池、事务管理

缓存与会话:
  缓存: Redis 6+ (单机模式)
  用途:
    - 限流计数器 (Token Bucket)
    - Refresh Token 存储
    - 会话数据缓存
    - 热点数据缓存
  客户端: aioredis 2.x (异步)

# ===== AI 服务集成 =====
阿里云百炼:
  提供商: 阿里云百炼智能体平台
  模型: 通义千问 (Qwen)
  功能:
    - 学习问答 (Chat Completion)
    - 作业批改 (批量处理)
    - 知识点提取 (NLP)
  架构: 单一智能体 + 场景参数化
  策略: MCP 精确查询 + 上下文增强
  超时: 120 秒 (支持图片 OCR)
  重试: 最多 3 次 (指数退避)

阿里云语音:
  服务: 实时语音识别 (ASR)
  格式: MP3, 16kHz
  特性: 智能断句、逆文本正则化
  最大时长: 60 秒

阿里云 OSS:
  存储桶: wuhao-tutor-files
  区域: cn-hangzhou
  用途: 用户头像、作业图片、文件上传
  最大文件: 10MB
  允许格式: jpg, jpeg, png, pdf, webp

# ===== Web 前端 =====
Web 前端:
  框架: Vue 3.4+ (Composition API)
  语言: TypeScript 5.6+
  UI 库: Element Plus 2.5+
  构建工具: Vite 5+ (ESM 构建)
  状态管理: Pinia 2.1+ (Vue 官方推荐)
  路由: Vue Router 4.x
  HTTP 客户端: Axios 1.6+

前端特性:
  数学公式: KaTeX 0.16+ (LaTeX 渲染)
  Markdown: marked 16.x + highlight.js
  图表: ECharts 6.0 + vue-echarts
  样式: Tailwind CSS + SCSS
  图片压缩: browser-image-compression
  日期处理: Day.js 1.11+

# ===== 微信小程序 =====
小程序端:
  语言: 原生 JavaScript + TypeScript 声明
  框架: 微信小程序原生 API
  UI: 微信原生组件 + 自定义组件
  网络: 自定义请求封装 (utils/request.js)
  状态: 全局 App 数据 + 页面 data
  缓存: wx.storage (本地存储)

小程序特性:
  API 地址: https://www.horsduroot.com (生产)
  超时: 120 秒 (支持图片上传和 AI 处理)
  重试: 3 次自动重试
  离线: 数据缓存 + 同步管理
  性能: 请求队列 + 优先级调度
  安全: Token 自动刷新 + 401 拦截

# ===== 开发与构建 =====
开发工具:
  Python:
    - uv (快速包管理，替代 pip)
    - Black (代码格式化，88 字符)
    - Flake8 (代码检查)
    - mypy (类型检查，strict 模式)
    - pytest (单元测试 + 覆盖率)

  Node.js:
    - npm (包管理)
    - ESLint (TS/Vue 代码检查)
    - Prettier (代码格式化)
    - Vitest (单元测试)
    - vue-tsc (类型检查)

版本控制:
  Git: 分支策略 (main/develop/feature/*)
  提交规范: Conventional Commits
  CI/CD: 计划集成 GitHub Actions

# ===== 部署与运维 =====
生产部署:
  服务器: 阿里云 ECS (121.199.173.244)
  域名: horsduroot.com (HTTPS)
  系统: Linux + systemd
  反向代理: Nginx 1.x
  SSL: Let's Encrypt (自动续期)
  进程管理: systemd (wuhao-tutor.service)
  日志: journalctl + 应用日志

部署目录:
  后端: /opt/wuhao-tutor
  前端: /var/www/html
  日志: /var/log/wuhao-tutor
  配置: /opt/wuhao-tutor/.env.production

监控告警:
  性能监控: 内置 MetricsCollector
  系统监控: SystemCollector (CPU/内存/磁盘)
  慢查询: 自动记录 (>1.0 秒)
  Prometheus: 配置就绪 (待启用)
```

### 四层架构设计 (严格分层)

```
┌──────────────────────────────────────────────────────┐
│  API Layer (src/api/v1/endpoints/)                  │
│  - 25+ 学习问答端点                                   │
│  - 10+ 错题管理端点                                   │
│  - 8+ 学习分析端点                                    │
│  - 5+ 用户认证端点                                    │
│  - 3+ 知识图谱端点                                    │
│  职责: HTTP 请求处理、参数验证、响应格式化            │
├──────────────────────────────────────────────────────┤
│  Service Layer (src/services/)                       │
│  - BailianService (AI 服务，1000+ 行)                │
│  - LearningService (问答业务，2400+ 行)              │
│  - MistakeService (错题业务，1260+ 行)               │
│  - KnowledgeGraphService (知识图谱)                  │
│  - AnalyticsService (学习分析)                        │
│  - AuthService (认证授权)                             │
│  职责: 业务逻辑、事务管理、多 Repository 组合         │
├──────────────────────────────────────────────────────┤
│  Repository Layer (src/repositories/)                │
│  - BaseRepository[T] (泛型基类)                       │
│  - MistakeRepository                                  │
│  - LearningRepository                                 │
│  - KnowledgeGraphRepository                           │
│  - AnalyticsRepository                                │
│  职责: 数据访问、复杂查询、数据转换                   │
├──────────────────────────────────────────────────────┤
│  Model Layer (src/models/)                           │
│  - BaseModel (通用基类)                               │
│  - User (用户，支持 K12 全学段)                       │
│  - MistakeRecord + MistakeReview (错题)              │
│  - ChatSession + Question + Answer (问答)            │
│  - KnowledgeNode + KnowledgeRelation (知识图谱)      │
│  - LearningAnalytics (学习分析)                       │
│  职责: ORM 映射、数据模型、关系定义                   │
└──────────────────────────────────────────────────────┘

核心基础设施 (src/core/):
├── config.py         # Pydantic Settings 配置管理
├── database.py       # 异步数据库连接池 (SQLAlchemy 2.x)
├── security.py       # JWT 认证 + 多维限流 (Token Bucket + Sliding Window)
├── monitoring.py     # 性能监控指标收集 (MetricsCollector + SystemCollector)
├── performance.py    # 慢查询监听 + N+1 检测
├── logging.py        # 结构化日志记录 (JSON 格式)
└── exceptions.py     # 统一异常定义 (20+ 异常类型)
```

**架构原则**:

- ✅ **严格分层**: API → Service → Repository → Model (单向依赖)
- ✅ **全异步**: 所有 I/O 操作使用 `async/await`
- ✅ **依赖注入**: FastAPI `Depends` 机制
- ✅ **类型安全**: 完整类型注解 + mypy strict 检查
- ✅ **单一职责**: 每个模块职责清晰，函数 ≤ 60 行
- ❌ **禁止跨层**: 禁止 API 直接调用 Repository

---

## 🚀 快速开始

### 环境要求

```bash
# 必需
Python 3.11+         # 后端运行环境
Node.js 18+          # 前端构建环境
uv                   # Python 快速包管理器 (推荐)

# 可选 (生产环境)
PostgreSQL 14+       # 主数据库
Redis 6+             # 缓存和限流
Nginx               # 反向代理
systemd             # 服务管理 (Linux)
```

### 安装步骤

#### 1. 克隆项目

```bash
git clone https://github.com/hordu-ma/wuhao-tutor.git
cd wuhao-tutor
```

#### 2. 后端环境配置

```bash
# 安装 uv (如果未安装)
curl -LsSf https://astral.sh/uv/install.sh | sh

# 使用 uv 同步依赖
uv sync

# 复制环境变量模板
cp config/templates/.env.development .env

# 编辑 .env 配置必要的环境变量:
# - SQLALCHEMY_DATABASE_URI (默认 SQLite, 生产用 PostgreSQL)
# - BAILIAN_APPLICATION_ID (阿里云百炼应用 ID)
# - BAILIAN_API_KEY (阿里云百炼 API Key, 生产必须 sk- 开头)
# - SECRET_KEY (JWT 密钥, 生产必须更改默认值)
# - REDIS_HOST (可选, 默认 localhost)
```

**关键配置说明**:

```env
# 数据库配置 (开发环境使用 SQLite)
SQLALCHEMY_DATABASE_URI=sqlite+aiosqlite:///./data/local/wuhao_tutor.db

# 生产环境使用 PostgreSQL
# SQLALCHEMY_DATABASE_URI=postgresql+asyncpg://user:pass@host:5432/db

# 阿里云百炼配置 (必填)
BAILIAN_APPLICATION_ID=your_app_id
BAILIAN_API_KEY=sk-your-api-key  # 生产环境必须 sk- 开头
BAILIAN_TIMEOUT=120              # 支持图片 OCR, 默认 120 秒

# Redis 配置 (可选, 用于限流和缓存)
REDIS_HOST=localhost
REDIS_PORT=6379
# REDIS_PASSWORD=your_password   # 生产环境设置密码

# JWT 配置
SECRET_KEY=your-secret-key-change-in-production  # 生产必须更改
ACCESS_TOKEN_EXPIRE_MINUTES=11520  # 8 天
REFRESH_TOKEN_EXPIRE_MINUTES=43200 # 30 天

# 文件上传配置
UPLOAD_DIR=./uploads
UPLOAD_MAX_SIZE=10485760  # 10MB
```

#### 3. 初始化数据库

```bash
# 运行数据库迁移 (创建所有表)
uv run alembic upgrade head

# 初始化示例数据 (可选, 用于开发测试)
uv run python scripts/init_database.py
```

**数据库表结构** (自动创建):

- `users` - 用户信息 (支持 K12 全学段)
- `chat_sessions` - 问答会话
- `questions` + `answers` - 问答记录
- `mistake_records` + `mistake_reviews` - 错题和复习
- `knowledge_nodes` + `knowledge_relations` - 知识图谱
- `learning_analytics` - 学习分析数据

#### 4. 启动后端服务

```bash
# 开发模式 (热重载)
uv run python src/main.py

# 或使用 Make 命令
make dev

# 或使用 uvicorn 命令 (手动指定参数)
uv run uvicorn src.main:app --reload --host 0.0.0.0 --port 8000
```

**后端服务启动信息**:

```
🚀 应用启动中...
✅ 性能监控已启动
✅ FastAPI 应用创建完成
INFO:     Uvicorn running on http://0.0.0.0:8000
INFO:     Application startup complete.
```

#### 5. 启动前端服务 (Web)

```bash
cd frontend

# 安装依赖 (首次运行)
npm install

# 启动开发服务器
npm run dev
```

**前端服务启动信息**:

```
VITE v5.1.4  ready in 1234 ms

➜  Local:   http://localhost:5173/
➜  Network: http://192.168.1.100:5173/
```

#### 6. 配置微信小程序 (可选)

**小程序配置文件**: `miniprogram/config/index.js`

```javascript
const config = {
  environment: 'development', // 或 'production'

  api: {
    // 开发环境连接本地
    baseUrl: 'http://localhost:8000',

    // 生产环境连接阿里云 (当前默认配置)
    // baseUrl: 'https://www.horsduroot.com',

    timeout: 120000, // 120 秒 (支持图片上传和 AI 处理)
  },

  // 其他配置...
}
```

**重要**:

- 当前小程序默认连接**生产环境** (`https://www.horsduroot.com`)
- 本地开发需要修改 `baseUrl` 为 `http://localhost:8000`
- 小程序需要在微信开发者工具中打开 `miniprogram/` 目录

#### 7. 访问应用

| 入口           | 地址                                        | 说明                  |
| -------------- | ------------------------------------------- | --------------------- |
| **Web 前端**   | http://localhost:5173                       | Vue3 应用主界面       |
| **API 文档**   | http://localhost:8000/docs                  | Swagger UI 交互式文档 |
| **ReDoc 文档** | http://localhost:8000/redoc                 | ReDoc 风格 API 文档   |
| **健康检查**   | http://localhost:8000/health                | 服务健康状态检查      |
| **性能指标**   | http://localhost:8000/api/v1/health/metrics | JSON 格式性能指标     |
| **微信小程序** | 微信开发者工具                              | 需要微信开发者账号    |

### 开发工具和命令

#### Make 命令 (推荐使用)

```bash
# 后端开发
make dev              # 启动开发服务器
make test             # 运行测试套件 + 覆盖率报告
make lint             # 代码格式化 (black + flake8)
make type-check       # mypy 类型检查

# 数据库管理
make db-init          # 执行 Alembic 迁移到最新版本
make db-migrate       # 创建新迁移 (需手动输入描述)
make db-reset         # 开发环境重置 (⚠️ 删除所有数据)
make db-backup        # PostgreSQL 生产备份

# 文档生成
make schema           # 导出 OpenAPI JSON 到 docs/api/openapi.json

# 清理
make clean            # 清理缓存、日志、临时文件
```

#### 手动脚本

```bash
# 环境诊断 (检查依赖、配置、数据库连接)
uv run python scripts/diagnose.py

# 数据库迁移
uv run alembic upgrade head          # 升级到最新版本
uv run alembic downgrade -1          # 回退一个版本
uv run alembic revision -m "描述"    # 创建新迁移

# 初始化数据
uv run python scripts/init_database.py

# 代码质量
uv run black src/ tests/             # 格式化代码
uv run flake8 src/ tests/            # 代码检查
uv run mypy src/                     # 类型检查
uv run pytest tests/ --cov=src/      # 测试 + 覆盖率
```

#### 前端命令

```bash
cd frontend

npm run dev           # 开发服务器
npm run build         # 生产构建
npm run preview       # 预览生产构建
npm run type-check    # TypeScript 类型检查
npm run lint          # ESLint 检查
npm run format        # Prettier 格式化
npm run test          # Vitest 单元测试
npm run test:coverage # 测试覆盖率报告
```

### 快速验证

#### 1. 测试后端 API

```bash
# 健康检查
curl http://localhost:8000/health

# 预期响应
{
  "status": "healthy",
  "version": "0.1.0",
  "environment": "development"
}

# 测试学习问答端点 (需要先注册/登录获取 token)
# 参见 API 文档: http://localhost:8000/docs
```

#### 2. 测试前端界面

1. 浏览器访问 http://localhost:5173
2. 注册新用户或使用测试账号
3. 测试核心功能:
   - 作业问答: 提问并查看 AI 回答
   - 错题手册: 添加错题、标记复习
   - 学习分析: 查看学习统计图表

#### 3. 测试微信小程序 (可选)

1. 微信开发者工具打开 `miniprogram/` 目录
2. 修改 `config/index.js` 连接本地后端
3. 编译运行并测试功能

### 常见问题

**问题 1**: `uv sync` 失败

```bash
# 解决方法: 清理缓存重试
uv cache clean
uv sync --no-cache
```

**问题 2**: 数据库迁移失败

```bash
# 开发环境: 删除数据库重新初始化
rm data/local/wuhao_tutor.db
uv run alembic upgrade head
```

**问题 3**: 前端无法连接后端

- 检查后端是否启动 (http://localhost:8000/health)
- 检查 `frontend/vite.config.ts` 代理配置
- 检查浏览器控制台 CORS 错误

**问题 4**: 微信小程序 API 调用失败

- 检查 `miniprogram/config/index.js` 的 `baseUrl`
- 确认小程序合法域名配置 (生产环境)
- 检查微信开发者工具不校验合法域名选项 (开发环境)

更多问题参见: [故障排查文档](docs/troubleshooting/)

---

## 📚 项目文档

### 开发文档

- [架构概览](docs/architecture/overview.md) - 四层架构设计、技术选型
- [数据访问层](docs/architecture/data-access.md) - Repository 模式、数据库设计
- [API 端点文档](docs/api/endpoints.md) - 完整 API 清单和使用说明
- [开发指南](docs/guide/development.md) - 环境搭建、开发流程、工具使用

### 部署文档

- **[一键部署脚本](scripts/DEPLOY-README.md)** ⭐ - 简单高效的生产部署工具
- [生产部署指南](docs/deployment/production-deployment-guide.md) - systemd + Nginx 完整部署流程
- [本地代码验证](docs/deployment/local-code-verification.md) - 部署前代码安全检查
- [RDS 数据库配置](docs/deployment/RDS_CONNECTION_GUIDE.md) - PostgreSQL 连接配置
- [Redis 缓存配置](docs/deployment/REDIS_CONNECTION_GUIDE.md) - Redis 连接和使用
- [安全密钥管理](docs/deployment/SECURITY_KEYS_GUIDE.md) - API Key 和密钥配置

### 集成文档

- [前端集成](docs/integration/frontend.md) - Vue3 集成方案、API 调用
- [微信小程序开发](docs/integration/wechat-miniprogram.md) - 小程序架构和开发规范
- [微信认证集成](docs/integration/wechat-auth.md) - 微信登录和用户绑定

---

## 🛠️ 开发规范

### 项目结构

```
wuhao-tutor/
├── src/                          # 后端源码
│   ├── api/v1/                   # API 路由层
│   │   ├── api.py               # 主路由聚合
│   │   └── endpoints/           # 各模块端点
│   │       ├── learning.py      # 学习问答 (25+ 端点)
│   │       ├── mistakes.py      # 错题管理 (10+ 端点)
│   │       ├── analytics.py     # 学习分析 (8+ 端点)
│   │       ├── knowledge_graph.py # 知识图谱
│   │       ├── auth.py          # 认证授权
│   │       ├── user.py          # 用户管理
│   │       └── file.py          # 文件上传
│   ├── services/                # 业务逻辑层
│   │   ├── bailian_service.py   # AI 服务 (1000+ 行)
│   │   ├── learning_service.py  # 问答业务 (2400+ 行)
│   │   ├── mistake_service.py   # 错题业务 (1260+ 行)
│   │   ├── knowledge_graph_service.py
│   │   ├── analytics_service.py
│   │   ├── auth_service.py
│   │   ├── user_service.py
│   │   ├── file_service.py
│   │   ├── formula_service.py   # 公式渲染
│   │   ├── ai_image_service.py  # 图片识别
│   │   └── speech_recognition_service.py  # 语音识别
│   ├── repositories/            # 数据访问层
│   │   ├── base_repository.py   # 泛型基类
│   │   ├── mistake_repository.py
│   │   ├── learning_repository.py
│   │   ├── knowledge_graph_repository.py
│   │   └── analytics_repository.py
│   ├── models/                  # ORM 数据模型
│   │   ├── base.py             # BaseModel (UUID 主键 + 时间戳)
│   │   ├── user.py             # 用户 (支持 K12 全学段)
│   │   ├── learning.py         # ChatSession, Question, Answer
│   │   ├── study.py            # MistakeRecord, MistakeReview
│   │   ├── knowledge.py        # KnowledgeNode, KnowledgeRelation
│   │   ├── knowledge_graph.py  # 知识图谱关联表
│   │   ├── answer_quality.py   # 答题质量评分
│   │   └── formula_cache.py    # 公式缓存
│   ├── schemas/                # Pydantic 数据模型
│   │   ├── learning.py
│   │   ├── mistake.py
│   │   ├── analytics.py
│   │   └── auth.py
│   ├── core/                   # 核心基础设施
│   │   ├── config.py          # Pydantic Settings 配置
│   │   ├── database.py        # 异步数据库连接
│   │   ├── security.py        # JWT + 限流
│   │   ├── monitoring.py      # 性能监控
│   │   ├── performance.py     # 慢查询监听
│   │   ├── logging.py         # 结构化日志
│   │   └── exceptions.py      # 统一异常
│   ├── utils/                 # 工具函数
│   │   ├── type_converters.py # 类型转换
│   │   └── cache.py           # 缓存装饰器
│   └── main.py                # FastAPI 应用入口
│
├── frontend/                   # Vue3 Web 前端
│   ├── src/
│   │   ├── components/        # Vue 组件
│   │   ├── views/             # 页面视图
│   │   ├── stores/            # Pinia 状态管理
│   │   ├── router/            # Vue Router
│   │   ├── api/               # API 客户端
│   │   ├── utils/             # 工具函数
│   │   └── styles/            # 全局样式
│   ├── public/                # 静态资源
│   ├── vite.config.ts         # Vite 配置
│   └── package.json
│
├── miniprogram/               # 微信小程序
│   ├── pages/                 # 页面 (15+ 个)
│   │   ├── index/            # 首页
│   │   ├── learning/         # 学习问答
│   │   ├── mistakes/         # 错题本
│   │   ├── analysis/         # 学习分析
│   │   └── profile/          # 个人中心
│   ├── components/            # 自定义组件
│   ├── utils/                 # 工具模块 (20+ 个)
│   │   ├── request.js        # 网络请求封装
│   │   ├── api.js            # API 客户端
│   │   ├── auth.js           # 认证管理
│   │   ├── cache-manager.js  # 缓存管理
│   │   └── error-handler.js  # 错误处理
│   ├── config/               # 配置文件
│   └── app.json              # 小程序配置
│
├── alembic/                   # 数据库迁移
│   ├── versions/             # 迁移脚本
│   └── env.py
│
├── scripts/                   # 开发脚本
│   ├── deploy.sh             # 一键部署 (生产)
│   ├── start-dev.sh          # 启动开发环境
│   ├── diagnose.py           # 环境诊断
│   ├── init_database.py      # 初始化数据
│   └── sql/                  # SQL 脚本
│
├── tests/                     # 测试套件
│   ├── unit/                 # 单元测试
│   ├── integration/          # 集成测试
│   └── performance/          # 性能测试
│
├── docs/                      # 项目文档
│   ├── api/                  # API 文档
│   ├── architecture/         # 架构设计
│   ├── deployment/           # 部署文档
│   ├── guide/                # 使用指南
│   └── troubleshooting/      # 故障排查
│
├── config/templates/          # 配置模板
│   ├── .env.development
│   ├── .env.production
│   └── .env.testing
│
├── deploy/systemd/           # systemd 服务配置
├── nginx/                    # Nginx 配置
├── monitoring/               # Prometheus 配置
├── data/                     # 本地数据 (SQLite, 知识库)
├── uploads/                  # 文件上传目录
├── logs/                     # 日志文件
│
├── pyproject.toml            # Python 项目配置
├── alembic.ini              # Alembic 配置
├── Makefile                 # Make 命令
├── README.md                # 项目说明
├── DEVELOPMENT_STATUS.md    # 开发状态
├── DEVELOPMENT_ROADMAP.md   # 开发路线图
└── CHANGELOG.md             # 更新日志
```

### Git 提交规范

**Conventional Commits 规范**:

```
类型(范围): 简洁描述

类型 (必填):
- feat:     新功能
- fix:      修复 bug
- docs:     文档更新
- refactor: 重构 (不影响功能)
- test:     测试相关
- chore:    构建/工具链更新
- perf:     性能优化
- style:    代码格式 (不影响逻辑)

范围 (可选):
- learning:  学习问答
- mistake:   错题手册
- analytics: 学习分析
- auth:      认证授权
- api:       API 层
- ui:        前端界面
- miniprogram: 微信小程序

示例:
feat(learning): 添加数学公式 KaTeX 渲染支持
fix(mistake): 修复艾宾浩斯复习算法间隔计算错误
docs(readme): 更新项目架构和功能说明
refactor(api): 重构错题 API 使用 BaseRepository
test(service): 添加 MistakeService 单元测试
chore(deps): 升级 FastAPI 到 0.104.1
perf(db): 优化知识图谱查询索引
```

**分支策略**:

```
main         # 生产环境主分支
develop      # 开发主分支
feature/*    # 功能分支 (从 develop 切出)
hotfix/*     # 紧急修复 (从 main 切出)
release/*    # 发布分支

工作流:
1. 从 develop 创建 feature/xxx
2. 开发并提交
3. PR 到 develop
4. Code Review
5. 合并到 develop
6. 测试通过后合并到 main
7. 打 tag 发布
```

### 代码质量标准

**Python 后端**:

```python
# 1. 类型注解 (必须)
async def create_mistake(
    self,
    db: AsyncSession,
    user_id: UUID,
    request: CreateMistakeRequest
) -> MistakeDetailResponse:
    """创建错题记录"""
    # 实现...

# 2. 错误处理 (具体异常)
try:
    result = await self.bailian_service.chat(messages)
except BailianTimeoutError:
    raise ServiceError("AI 服务超时")
except BailianAuthError:
    raise ServiceError("AI 服务认证失败")

# 3. 单一职责 (函数 ≤ 60 行)
async def _calculate_mastery_score(
    self,
    review_count: int,
    correct_rate: float
) -> int:
    """计算掌握度分数"""
    # 简洁实现...
    return score

# 4. 文档注释 (Google 风格)
async def get_review_schedule(
    self,
    user_id: UUID,
    subject: Optional[str] = None
) -> List[ReviewTask]:
    """
    获取复习计划

    Args:
        user_id: 用户 ID
        subject: 学科过滤 (可选)

    Returns:
        List[ReviewTask]: 待复习任务列表

    Raises:
        NotFoundError: 用户不存在
    """
    pass
```

**代码检查工具**:

```bash
# 格式化 (自动修复)
uv run black src/ tests/ --line-length 88

# 代码检查
uv run flake8 src/ tests/ --max-line-length 88

# 类型检查 (strict 模式)
uv run mypy src/ --strict

# 测试 + 覆盖率
uv run pytest tests/ --cov=src/ --cov-report=html
```

**TypeScript 前端**:

```typescript
// 1. 类型定义 (必须)
interface AskQuestionRequest {
  session_id: string
  content: string
  images?: string[]
}

// 2. Composition API (推荐)
import { ref, computed, onMounted } from 'vue'

export default defineComponent({
  setup() {
    const questions = ref<Question[]>([])
    const loading = ref(false)

    const fetchQuestions = async () => {
      loading.value = true
      try {
        questions.value = await api.getQuestions()
      } catch (error) {
        handleError(error)
      } finally {
        loading.value = false
      }
    }

    onMounted(fetchQuestions)

    return { questions, loading, fetchQuestions }
  },
})

// 3. 错误处理
try {
  await api.askQuestion(request)
} catch (error) {
  if (error.code === 'AUTH_ERROR') {
    router.push('/login')
  } else {
    ElMessage.error(error.message)
  }
}
```

**前端代码检查**:

```bash
# TypeScript 类型检查
npm run type-check

# ESLint 检查
npm run lint

# Prettier 格式化
npm run format

# 单元测试
npm run test
```

### 架构规范

**1. 严格分层 (禁止跨层调用)**:

```python
✅ 正确:
API → Service → Repository → Model

❌ 错误:
API → Repository  # 跳过 Service 层
Service → Model  # 跳过 Repository 层
```

**2. 依赖注入 (FastAPI Depends)**:

```python
from fastapi import Depends
from src.core.dependencies import get_db, get_current_user

@router.post("/ask")
async def ask_question(
    request: AskQuestionRequest,
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user)
) -> AskQuestionResponse:
    service = LearningService(db)
    return await service.ask_question(current_user.id, request)
```

**3. 异步优先 (async/await)**:

```python
# ✅ 所有 I/O 操作使用 async
async def get_mistakes(self, user_id: UUID) -> List[Mistake]:
    return await self.repository.find_by_user(user_id)

# ❌ 禁止阻塞调用
def get_mistakes(self, user_id: UUID) -> List[Mistake]:
    return self.repository.find_by_user(user_id)  # 阻塞!
```

**4. 配置外化 (环境变量)**:

```python
# ✅ 使用环境变量
from src.core.config import get_settings
settings = get_settings()
api_key = settings.BAILIAN_API_KEY

# ❌ 禁止硬编码
api_key = "sk-xxxxxxxxxxxx"  # 危险!
```

### 测试策略

**单元测试** (Service 和 Repository):

```python
import pytest
from uuid import uuid4

@pytest.mark.asyncio
async def test_create_mistake(mistake_service):
    """测试创建错题"""
    request = CreateMistakeRequest(
        question_content="测试题目",
        subject="math",
        difficulty=3
    )

    result = await mistake_service.create_mistake(
        user_id=uuid4(),
        request=request
    )

    assert result.id is not None
    assert result.subject == "math"
    assert result.difficulty == 3
```

**集成测试** (API 端点):

```python
@pytest.mark.asyncio
async def test_get_mistakes_list(client, auth_headers):
    """测试错题列表接口"""
    response = await client.get(
        "/api/v1/mistakes",
        headers=auth_headers
    )

    assert response.status_code == 200
    data = response.json()
    assert "items" in data
    assert "total" in data
```

**性能测试** (基准测试):

```python
@pytest.mark.benchmark
def test_knowledge_graph_query_performance(benchmark):
    """测试知识图谱查询性能"""
    result = benchmark(
        lambda: get_knowledge_graph(user_id)
    )
    # 断言 P95 < 100ms
    assert result.stats.percentile_95 < 0.1
```

---

## 📊 项目统计

```
代码规模:
- Python 后端: ~20,000 行 (含服务、模型、API)
  - 核心服务: ~8,000 行 (BailianService, LearningService, MistakeService 等)
  - 数据模型: ~3,000 行 (15+ 个数据表)
  - API 端点: ~4,000 行 (50+ 个端点)
  - 基础设施: ~3,000 行 (config, security, monitoring, logging)
  - 工具脚本: ~2,000 行

- Vue 前端: ~12,000 行
  - 组件: ~6,000 行 (问答、错题、分析等)
  - Stores: ~2,000 行 (Pinia 状态管理)
  - 路由: ~1,000 行
  - 工具: ~1,500 行
  - 样式: ~1,500 行

- 微信小程序: ~8,000 行
  - 页面: ~4,000 行 (15+ 个页面)
  - 组件: ~2,000 行
  - 工具: ~2,000 行 (网络、缓存、认证等)

- 测试代码: ~4,000 行
  - 单元测试: ~2,500 行
  - 集成测试: ~1,000 行
  - 性能测试: ~500 行

总计: ~44,000 行代码

数据库表:
- 核心表: 15+ 个
  - users (用户)
  - chat_sessions, questions, answers (问答)
  - mistake_records, mistake_reviews (错题)
  - knowledge_nodes, knowledge_relations (知识图谱)
  - learning_analytics (学习分析)
  - answer_quality_scores (答题质量)
  - formula_cache (公式缓存)

API 端点统计:
- 学习问答: 25+ 个端点
- 错题管理: 10+ 个端点
- 学习分析: 8+ 个端点
- 知识图谱: 3+ 个端点
- 用户认证: 5+ 个端点
- 文件上传: 3+ 个端点
- 健康检查: 2+ 个端点
总计: 50+ 个 RESTful API 端点

测试覆盖率 (pytest + pytest-cov):
- 核心模块: 80%+
- Service 层: 85%+
- Repository 层: 90%+
- API 层: 75%+
- 总体覆盖: ~82%

性能指标 (生产环境实测):
- API 响应时间: P50 < 100ms, P95 < 200ms, P99 < 500ms
- 数据库查询: P50 < 20ms, P95 < 50ms, P99 < 100ms
- AI 问答延迟: P50 < 2s, P95 < 3s, P99 < 5s (不含流式)
- 慢查询监控: 自动记录 > 1.0 秒的查询
- 并发处理: 支持 100+ 并发请求 (Uvicorn workers)

限流保护 (实际配置):
- IP 层: 100 请求/分钟
- 用户层: 50 请求/分钟
- AI 服务: 20 请求/分钟
- 登录端点: 10 请求/分钟

依赖版本:
- Python 核心依赖: 30+ 个
- Vue 前端依赖: 40+ 个
- 小程序工具: 20+ 个自定义工具模块
```

---

## 🔮 未来规划

详见 [开发计划](DEVELOPMENT_ROADMAP.md) 和 [开发进度](DEVELOPMENT_STATUS.md)

### 短期目标 (1-2 个月)

**1. 向量数据库集成 (PGVector)**

- 集成 PostgreSQL pgvector 扩展
- 错题语义相似度检索
- 相似问题推荐
- 知识点聚类分析

**2. 推荐系统基础版**

- 协同过滤算法
- 基于学习记录的练习推荐
- 基于知识点掌握度的学习路径推荐
- 个性化题目难度调整

**3. 知识图谱增强**

- 前端 ECharts 可视化优化
- 知识点层级关系展示
- 学习路径交互式规划
- 薄弱点自动识别和提醒

**4. 小程序功能增强**

- 消息推送 (复习提醒、学习报告)
- 离线缓存优化
- 图片压缩优化
- 语音输入体验改进

**5. 性能优化**

- Redis 缓存策略优化
- 数据库索引优化
- N+1 查询消除
- CDN 静态资源加速

### 中期目标 (3-6 个月)

**1. RAG 语义增强**

- 向量化知识库构建
- 语义检索 + 关键词检索混合
- 上下文窗口优化
- 提示词工程改进

**2. 协作学习功能**

- 学习小组创建和管理
- 问题分享和讨论
- 互助答疑系统
- 学习排行榜

**3. 多模态输入增强**

- 手写识别 (数学公式、中文)
- 图片 OCR 准确率提升
- 语音识别准确率优化
- 视频讲解支持

**4. 学习报告生成**

- 周报/月报自动生成
- 学习趋势分析
- 家长端查看
- 教师端管理

**5. 数据分析增强**

- 学习行为分析
- 知识点薄弱环节识别
- 学习效率评估
- 个性化学习建议

### 长期愿景 (6-12 个月)

**1. 多学科扩展**

- 从数学扩展到语文、英语
- 物理、化学、生物学科支持
- 各学科知识图谱构建
- 跨学科知识关联

**2. 自适应学习**

- 动态难度调整
- 学习路径自适应
- 个性化推荐引擎
- 学习风格识别

**3. 教师管理端**

- 班级管理
- 作业批改
- 学情分析
- 教学资源库

**4. 开放平台**

- 提供 API 接口
- 支持第三方集成
- 插件系统
- 开发者社区

**5. 移动端原生应用**

- iOS App (Swift)
- Android App (Kotlin)
- 跨平台体验优化
- 离线模式增强

### 技术债务和优化

**近期计划**:

- [ ] Prometheus 监控集成
- [ ] Grafana 仪表板配置
- [ ] CI/CD 流水线搭建 (GitHub Actions)
- [ ] 前端单元测试覆盖率提升
- [ ] API 文档自动生成优化
- [ ] 数据库连接池优化
- [ ] 异常处理标准化
- [ ] 日志脱敏规则完善

**中期计划**:

- [ ] 微服务架构探索
- [ ] Kubernetes 部署方案
- [ ] 分布式追踪 (OpenTelemetry)
- [ ] A/B 测试框架
- [ ] 灰度发布机制
- [ ] 数据备份自动化
- [ ] 灾难恢复演练

---

## 🤝 贡献指南

### 如何贡献

1. Fork 本项目
2. 创建特性分支 (`git checkout -b feature/AmazingFeature`)
3. 提交更改 (`git commit -m 'feat: Add some AmazingFeature'`)
4. 推送到分支 (`git push origin feature/AmazingFeature`)
5. 提交 Pull Request

### 开发流程

1. 在 Issues 中讨论新功能或 Bug
2. 获得认可后开始开发
3. 确保代码通过所有测试
4. 更新相关文档
5. 提交 PR 并等待 Review

---

## 📄 许可证

本项目采用 MIT 许可证 - 详见 [LICENSE](LICENSE) 文件

---

## 👥 维护者

- **hordu-ma** - [GitHub](https://github.com/hordu-ma)

---

## 📮 联系方式

- 项目地址: https://github.com/hordu-ma/wuhao-tutor
- 问题反馈: https://github.com/hordu-ma/wuhao-tutor/issues
- 生产环境: https://www.horsduroot.com
- 健康检查: https://horsduroot.com/health

---

<div align="center">

**⭐ 如果这个项目对你有帮助,请给它一个 Star! ⭐**

Made with ❤️ by hordu-ma

</div>
