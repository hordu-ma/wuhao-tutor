...
class TestRateLimitRule:
    ...
    def test_rate_limit_rule_creation(self):
        """测试创建限流规则"""
        rule = RateLimitRule(
            name="api_limit", max_requests=100, window_seconds=60, scope="global"
        )
...
class TestRateLimiter:
    ...
    def test_add_custom_rule(self):
        ...
        custom_rule = RateLimitRule(
            name="custom_limit", max_requests=50, window_seconds=30, scope="per_user"
        )
    ...
    def test_is_allowed_over_limit(self):
        ...
        strict_rule = RateLimitRule(
            name="strict", max_requests=2, window_seconds=60, scope="per_ip"
        )
...
class TestRateLimitMiddleware:
    ...
    async def test_middleware_blocks_rate_limited_request(self):
        ...
        mock_rule = RateLimitRule(
            name="test", max_requests=1, window_seconds=60, scope="per_ip"
        )
        ...
        with patch.object(
            limiter, "is_allowed", return_value=(False, mock_rule, {"remaining": 0})
        ):
            result = await middleware.dispatch(mock_request, mock_call_next)
