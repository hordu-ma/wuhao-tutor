...
class TestAuthServiceTokenVerification:
    ...
    def test_verify_expired_token(self):
        ...
        jti = "session_123"
        # 创建已过期的token
        token = auth_service.create_access_token(user_id, jti, timedelta(seconds=-1))
...
class TestAuthServiceUserSession:
    ...
    async def test_verify_user_session_valid(self):
        ...
        with patch.object(
            auth_service, "_get_session", new=AsyncMock(return_value=mock_session)
        ):
    ...
    async def test_verify_user_session_invalid(self):
        ...
        with patch.object(
            auth_service, "_get_session", new=AsyncMock(return_value=None)
        ):
            result = await auth_service.verify_user_session(jti)
    ...
    async def test_invalidate_session(self):
        ...
        with patch.object(
            auth_service, "_invalidate_session_by_jti", new=AsyncMock()
        ) as mock_invalidate:
    ...
    async def test_invalidate_all_user_sessions(self):
        ...
        with patch.object(
            auth_service, "_invalidate_all_sessions_by_user", new=AsyncMock()
        ) as mock_invalidate:
