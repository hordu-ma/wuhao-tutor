...
| Utils         | `src/utils/`             | 通用工具、格式/ID 生成                   | 跨上下文副作用                   |
...
**触发条件**：当用户历史数据达到一定规模(> 100 条问答记录)，且 MCP 上下文效果出现瓶颈时启动。
...
- **Embedding API 调用** - 通义千问约 ¥0.0007/千 token (需评估月成本)
...
BAILIAN_CONFIG = {
    ...
    "api_key": "sk-xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx",  # 示例，生产需替换
