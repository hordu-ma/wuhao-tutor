...
class DiagnosticRunner:
    ...
    def test_module_imports(self) -> None:
        ...
        message = "所有模块导入成功" if success else f"{len(failed_imports)}个模块导入失败"
    ...
    def test_fastapi_app(self) -> None:
        ...
        try:
            ...
            self.results.append(("FastAPI应用", True, f"应用创建成功，{route_count}个路由"))
    ...
    def test_services(self) -> None:
        ...
        try:
            ...
            if hasattr(bailian_service, 'config'):
                print(f"    - API Key: {'已配置' if bailian_service.config.api_key else '未配置'}")
    ...
    def print_summary(self) -> None:
        ...
        print("\\n状态: " + ("🟢 所有检查通过" if failed == 0 else "🟡 部分检查失败" if failed < total // 2 else "🔴 多项检查失败"))
