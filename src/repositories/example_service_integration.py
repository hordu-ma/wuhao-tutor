...
class EnhancedLearningService:
    ...
    async def get_or_create_active_session(
        ...
        try:
            ...
            if active_session:
                # 更新最后活跃时间
                await self.session_repo.update(active_session.id, {
    ...
    async def get_comprehensive_learning_analytics(
        ...
        try:
            ...
            analytics_response = LearningAnalyticsResponse(
                ...
                # 基础统计
                total_questions=learning_stats['total_questions'],
                total_sessions=learning_stats['total_sessions'],
                active_subjects=learning_stats['active_subjects'],
                avg_rating=learning_stats['avg_rating'],
                helpful_answers=learning_stats['helpful_answers'],

                # 分布统计
                subject_distribution=learning_stats['subject_distribution'],
                question_type_distribution=learning_stats['question_type_distribution'],

                # 活动模式
                daily_activity_pattern=daily_pattern,

                # 知识掌握
                topics_analyzed=mastery_analysis['topics_analyzed'],
                mastery_distribution=mastery_analysis['mastery_distribution'],
                strong_areas=[t['topic'] for t in mastery_analysis['strong_areas']],
                improvement_areas=[t['topic'] for t in mastery_analysis['improvement_areas']],

                # 改进建议
                low_rated_questions_count=len(low_rated_qa),
    ...
    async def _update_analytics_record(
        ...
        try:
            ...
            analytics_data = {
                ...
                'active_subjects': analytics_response.subject_distribution,
                'question_types_stats': analytics_response.question_type_distribution,
                'preferred_subjects': analytics_response.strong_areas,
                'avg_rating': analytics_response.avg_rating,
                'positive_feedback_rate': analytics_response.helpful_answers,
                'weekly_pattern': analytics_response.daily_activity_pattern,
                'knowledge_mastery': analytics_response.mastery_distribution,
                'improvement_areas': analytics_response.improvement_areas,
                'last_analyzed_at': datetime.now().isoformat()
            }

            if existing_analytics:
                # 更新现有记录
                await self.analytics_repo.update(existing_analytics.id, analytics_data)
            else:
                # 创建新记录
