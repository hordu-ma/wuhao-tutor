...
class StudyPreference(BaseModel):
    ...
    difficulty_preference: Optional[int] = Field(None, ge=1, le=5, description="难度偏好")
...
class UserProfile(UserBase):
    """用户个人资料"""
    study_subjects: Optional[List[SubjectType]] = Field(default_factory=list, description="学习学科")
    study_goals: Optional[List[LearningGoal]] = Field(default_factory=list, description="学习目标")
...
class UserProgressResponse(BaseModel):
    ...
    subject_progress: Dict[str, int] = Field(default_factory=dict, description="各学科进度")
...
class UserPerformanceReport(BaseModel):
    ...
    areas_for_improvement: List[str] = Field(default_factory=list, description="待改进领域")
...
class UserRelationship(BaseModel):
    ...
    relationship_type: str = Field(..., description="关系类型: parent_child/teacher_student")
...
class UserRelationshipResponse(BaseModel):
    """用户关系响应"""
    user: UserResponse
    relationships: List[UserRelationship] = Field(default_factory=list, description="用户关系列表")


class AddRelationshipRequest(BaseModel):
    """添加关系请求"""
    related_user_phone: str = Field(..., pattern=r"^1[3-9]\d{9}$", description="关联用户手机号")
