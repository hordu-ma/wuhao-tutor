...
class ChatMessage(BaseModel):
    ...
    role: MessageRole
    content: str
    image_urls: Optional[List[str]] = None  # 支持多模态消息（图片URL列表）
...
class HomeworkCorrectionResponse(BaseModel):
    ...
    knowledge_points: List[str] = Field(
        default_factory=list, description="涉及的所有知识点"
    )
...
class StudyQARequest(BaseModel):
    ...
    context_homework_ids: List[str] = Field(
        default_factory=list, description="相关作业ID列表"
    )
...
class StudyQAResponse(BaseModel):
    ...
    learning_resources: List[Dict[str, str]] = Field(
        default_factory=list, description="学习资源推荐"
    )
...
class LearningAnalysisRequest(BaseModel):
    ...
    user_id: str = Field(..., description="用户ID")
    time_range: AnalysisTimeRange = Field(
        default=AnalysisTimeRange.MONTH, description="分析时间范围"
    )
...
class LearningAnalysisResponse(BaseModel):
    ...
    # 学习建议
    study_suggestions: List[str] = Field(default_factory=list, description="学习建议")
    review_plan: List[Dict[str, Any]] = Field(
        default_factory=list, description="复习计划"
    )
    next_focus_areas: List[str] = Field(
        default_factory=list, description="下阶段重点领域"
    )

    # 趋势数据
    progress_trend: Dict[str, Any] = Field(
        default_factory=dict, description="进步趋势数据"
    )
    performance_comparison: Dict[str, Any] = Field(
        default_factory=dict, description="表现对比"
    )
