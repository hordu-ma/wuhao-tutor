...
class User(BaseModel):
    ...
    wechat_openid = Column(
        String(128), unique=True, index=True, nullable=True, comment="微信OpenID"
    )

    wechat_unionid = Column(
        String(128), unique=True, index=True, nullable=True, comment="微信UnionID"
    )
    ...
    institution = Column(String(100), nullable=True, comment="所属机构")

    # 联系信息
    parent_contact = Column(String(11), nullable=True, comment="家长联系电话")

    parent_name = Column(String(50), nullable=True, comment="家长姓名")

    # 系统信息
    role = Column(String(20), default="student", nullable=False, comment="用户角色")

    is_active = Column(Boolean, default=True, nullable=False, comment="是否激活")

    is_verified = Column(
        Boolean, default=False, nullable=False, comment="是否已验证手机号"
    )

    # 学习偏好设置
    study_subjects = Column(Text, nullable=True, comment="关注学科（JSON格式）")

    study_goals = Column(Text, nullable=True, comment="学习目标（JSON格式）")
...
class UserSession(BaseModel):
    ...
    device_type = Column(
        String(20), nullable=True, comment="设备类型 (web/mobile/mini_program)"
    )

    access_token_jti = Column(
        String(36), nullable=False, unique=True, comment="访问令牌JTI"
    )

    refresh_token_jti = Column(
        String(36), nullable=False, unique=True, comment="刷新令牌JTI"
    )
    ...
    def __repr__(self) -> str:
        return (
            f"<UserSession(user_id='{self.user_id}', device_type='{self.device_type}')>"
        )
