...
class RevisionPlan(BaseModel):
    ...
    user_id = Column(PG_UUID(as_uuid=True), ForeignKey("users.id"), nullable=False, index=True, comment="用户ID")
    
    # 元数据
    title = Column(String(255), nullable=False, comment="计划标题")
    description = Column(Text, nullable=True, comment="简短描述")
    cycle_type = Column(String(20), nullable=False, comment="周期类型: 7days|14days|30days")
    status = Column(String(20), default="draft", nullable=False, comment="状态: draft|published|completed|expired")
