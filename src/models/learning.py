...
class LearningAnalytics(BaseModel):
    ...
    active_subjects = Column(
        Text,
        nullable=True,
        comment="活跃学科统计(JSON格式)"
    )

    question_types_stats = Column(
        Text,
        nullable=True,
        comment="问题类型统计(JSON格式)"
    )

    # 学习特征
    preferred_subjects = Column(
        Text,
        nullable=True,
        comment="偏好学科(JSON格式)"
    )
    ...
    # 时间模式
    most_active_time = Column(
        String(20),
        nullable=True,
        comment="最活跃时间段"
    )

    weekly_pattern = Column(
        Text,
        nullable=True,
        comment="周学习模式(JSON格式)"
    )

    # 学习进度
    knowledge_mastery = Column(
        Text,
        nullable=True,
        comment="知识掌握度(JSON格式)"
    )

    improvement_areas = Column(
        Text,
        nullable=True,
        comment="待提升领域(JSON格式)"
    )
