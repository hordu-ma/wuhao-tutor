...
from src.core.exceptions import (
    ServiceError, BailianServiceError, NotFoundError, ValidationError
)
...
from src.models.homework import HomeworkSubmission, HomeworkReview
from src.schemas.learning import (
    AskQuestionRequest, AskQuestionResponse, CreateSessionRequest,
    SessionResponse, QuestionResponse, AnswerResponse,
    LearningAnalyticsResponse, FeedbackRequest,
    SessionListQuery, QuestionHistoryQuery, PaginatedResponse
)
from src.services.bailian_service import (
    BailianService, ChatMessage, MessageRole, AIContext,
    get_bailian_service
)
...
class LearningService:
    ...
    async def _save_question(
        ...
        question_data = {
            ...
            "context_data": json.dumps(request.context_data) if request.context_data else None,
    ...
    def _parse_grade_level(self, grade_level: Optional[str]) -> Optional[int]:
        ...
        grade_mapping = {
            "junior_1": 7, "junior_2": 8, "junior_3": 9,
            "senior_1": 10, "senior_2": 11, "senior_3": 12
        }
    ...
    async def _get_homework_context(
        ...
        try:
            # 获取最近的作业记录
            stmt = select(HomeworkSubmission).options(
                selectinload(HomeworkSubmission.reviews)
            ).where(
                HomeworkSubmission.user_id == user_id
            ).order_by(desc(HomeworkSubmission.created_at)).limit(5)
    ...
    async def _get_conversation_history(
        ...
        try:
            ...
            ).order_by(desc(Question.created_at)).limit(max_count)
    ...
    async def get_session_list(
        ...
        # 查询数据
        stmt = select(ChatSession).where(and_(*conditions)).order_by(
            desc(ChatSession.last_active_at)
    ...
    async def get_question_history(
        ...
        for question in questions:
            item = {
                "question": QuestionResponse.model_validate(question),
                "answer": AnswerResponse.model_validate(question.answer) if question.answer else None
            }
            items.append(item)
    ...
    async def get_learning_analytics(self, user_id: str) -> Optional[LearningAnalyticsResponse]:
        """获取学习分析"""
        analytics = await self.analytics_repo.get_by_field("user_id", user_id)
        if not analytics:
            return None
        ...
        # 计算平均评分
        avg_rating_stmt = select(func.avg(Answer.user_rating)).select_from(
            Answer.join(Question, Answer.question_id == Question.id)
    ...
    async def _calculate_positive_feedback_rate(self, user_id: str) -> int:
        """计算正面反馈率"""
        total_stmt = select(func.count(Answer.id)).select_from(
            Answer.join(Question, Answer.question_id == Question.id)
        ).where(Question.user_id == user_id, Answer.is_helpful.isnot(None))

        positive_stmt = select(func.count(Answer.id)).select_from(
            Answer.join(Question, Answer.question_id == Question.id)
    ...
    async def _generate_improvement_suggestions(
        ...
        for stat in subject_stats:
            if stat['question_count'] < 5:
                suggestions.append(f"建议增加{self._get_subject_name(stat['subject'])}学科的练习")

            if stat['avg_difficulty'] < 2.5:
                suggestions.append(f"可以尝试{self._get_subject_name(stat['subject'])}更有挑战性的问题")
    ...
    async def _identify_knowledge_gaps(self, user_id: str) -> List[str]:
        ...
        # 从问题话题中分析
        stmt = select(Question.topic).where(
            Question.user_id == user_id,
